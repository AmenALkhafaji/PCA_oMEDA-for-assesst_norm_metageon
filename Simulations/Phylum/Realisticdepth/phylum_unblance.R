...
max_depth_healthy <- 30000

min_depth_unhealthy <- 30000
max_depth_unhealthy <- 90000
...
prob_unhealthy <- c(45, 40, 1.15, 3, 10)
...
# -------------------------
# ANCOM-derived ALR transformed output for low-dimensional phylum data
# ANCOM = ALR-style log-ratio matrix using a data-driven reference taxon
#
# Critical note:
# ANCOM-BC is not statistically appropriate here because this phylum-level table
# has only 5 taxa. ANCOMBC itself warns that results are unreliable when the
# number of taxa is too small, e.g. < 10.
#
# Therefore, for phylum-level benchmarking, this function does NOT call ANCOMBC.
# It exports an ALR-style transformed matrix only, preserving the same number of
# columns as the input by setting the reference taxon column to 0.
# -------------------------
ancombc_logcorr_only_taxa <- function(
  ...
  if (any(X < 0)) stop("Counts must be non-negative.")
  if (any(rowSums(X) == 0)) {
    stop("At least one sample has total count zero; ALR cannot be computed.")
  }
  ...
  if (!reference_taxon %in% taxa_cols) {
    stop("reference_taxon must be one of taxa_cols.")
  }
  
  message(
    "ANCOM phylum output: using ALR-style transform instead of ANCOM-BC ",
    "because only ", length(taxa_cols), " taxa are available. ",
    "Reference taxon = ", reference_taxon, "."
  )
  ...
  # Preserve original matrix width for downstream PCA/oMEDA.
  # The reference column is log(ref/ref) = 0.
  X_alr[, reference_taxon] <- 0
  
  out <- as.data.frame(X_alr)
  colnames(out) <- taxa_cols
  rownames(out) <- rownames(df)
...
for (r in 1:R) {
  ...
  # ANCOM: ALR-style transformed output for phylum-level table
