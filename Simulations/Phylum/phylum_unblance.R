...
phyla <- c("Bacillota", "Bacteroidetes", "Actinobacteria",
...
for (r in 1:R) {
  ...
  healthy_df <- simulate_group(
    n = num_samples, probs = prob_healthy, group_label = 1,
    min_depth = min_depth_healthy, max_depth = max_depth_healthy
  )
  
  unhealthy_df <- simulate_group(
    n = num_samples, probs = prob_unhealthy, group_label = 2,
    min_depth = min_depth_unhealthy, max_depth = max_depth_unhealthy
  )
